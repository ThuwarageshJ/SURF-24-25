...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* event)
{
  ...
  G4double targetSizeXY = 0;
  G4double targetSizeZ = 0;

  if (!fTarget){
    G4LogicalVolume* targetLV = G4LogicalVolumeStore::GetInstance()->GetVolume("Target");
    if (targetLV) fTarget = dynamic_cast<G4Box*>(targetLV->GetSolid());
  }
  if (fTarget) {
    targetSizeXY = fTarget->GetXHalfLength() * 2.;
    targetSizeZ = fTarget->GetZHalfLength() * 2.;
  }
  else {
    G4ExceptionDescription msg;
  ...
  G4double xwidth = 8*cm;
  G4double ywidth = 3*cm;
